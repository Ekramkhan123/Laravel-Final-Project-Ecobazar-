    ...
    <div class="content-wrapper">
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center">
                ...
                <a class="btn btn-primary" href="{{ route('dashboard.product.image.show') }}">Show Images</a>
            </div>
                ...
                <form action="{{ route('dashboard.product.image.update', $findImages->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="row">
                        <div class="col-lg-6 mb-2">
                            ...
                            <div class="row">
                                @forelse ($findImages->images as $img)
                                    <div class="col-lg-3 card p-1">
                                        <div class="card-header d-flex justify-content-center align-items-center">
                                            <a onclick="return confirm('Are you sure ?')" class="btn btn-sm action_btn" href="{{ route('dashboard.product.image.delete', $img->id) }}"><span><iconify-icon icon="entypo:circle-with-cross" width="25" height="25"></iconify-icon></span></a>
                                        </div>
                                        <div class="card-body">
                                            <img class="img-fluid" width="100" height="100" src="{{ asset('storage/' . $img->image) }}" alt="">
                                        </div>
                                    </div>
                                @empty
                        ...
                        <div class="col-lg-6">
                            ...
                            <select class="js-example-basic-single form-control" name="product_id">
                                ...
                                @foreach ($products as $product)
                                    <option value="{{ $product->id }}" {{ $product->id == $findImages-> id ? 'selected' : 'none' }}>{{ $product->title }}</option>
                                @endforeach
                            </select>
                            @error('product_id')
                                <p class="text-danger">{{ $message }}</p>
